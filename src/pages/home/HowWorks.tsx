import {
  MdCheckCircleOutline,
  MdOutlineCalendarMonth,
  MdPeopleOutline,
} from 'react-icons/md'
import IconCircle from '../../shared/components/IconCircle'
import { motion, stagger } from 'motion/react'
import useStore from '../../store/useStore'
import { NAVIGATION } from '../../shared/const/navigation'

const HowWorks = () => {
  const container = {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      transition: {
        delayChildren: stagger(0.1),
      },
    },
  }

  const item = {
    hidden: { opacity: 0 },
    show: { opacity: 1 },
  }

  const { updateCurrentSection } = useStore()

  return (
    <motion.section
      className='flex flex-col gap-10 py-10 px-4 sm:py-20 sm:px-20 lg:gap-16 xl:px-40'
      onViewportEnter={() => updateCurrentSection(NAVIGATION.HOW_WORKS)}
      viewport={{ amount: 0.8 }}
    >
      <h1 className='subtitle scroll-mt-25' id={NAVIGATION.HOW_WORKS}>
        Cuidar a tu mascota nunca fue tan fácil
      </h1>

      <motion.div
        className='flex flex-col lg:max-w-1/2 lg:mx-auto'
        variants={container}
        initial='hidden'
        whileInView='show'
        viewport={{ amount: 0.9, once: true }}
      >
        <motion.div
          className='flex justify-between gap-4 lg:gap-6'
          variants={item}
        >
          <div className='flex items-center justify-center'>
            <IconCircle>
              <MdOutlineCalendarMonth className='text-primary size-6 lg:size-8' />
            </IconCircle>
          </div>

          <div className='flex flex-col gap-1 w-5/6'>
            <h2 className='text-sm font-medium lg:text-lg'>
              1. Busca el servicio que necesitas
            </h2>

            <p className='text-xs lg:text-base'>
              Paseos, cuidado por horas, baño, peluquería o atención especial.
            </p>
          </div>
        </motion.div>

        <motion.div
          className='flex justify-center items-center after:border after:h-8 after:my-4'
          variants={item}
        ></motion.div>

        <motion.div
          className='flex justify-between gap-4 lg:gap-6'
          variants={item}
        >
          <div className='flex items-center justify-center'>
            <IconCircle>
              <MdPeopleOutline className='text-primary size-6 lg:size-8' />
            </IconCircle>
          </div>

          <div className='flex flex-col gap-1 w-5/6'>
            <h2 className='text-sm font-medium lg:text-lg'>
              2. Elige un cuidador verificado
            </h2>

            <p className='text-xs lg:text-base'>
              Revisa perfiles, calificaciones y experiencia.
            </p>
          </div>
        </motion.div>

        <motion.div
          className='flex justify-center items-center after:border after:h-8 after:my-4'
          variants={item}
        ></motion.div>

        <motion.div
          className='flex justify-between items-center gap-4 lg:gap-6'
          variants={item}
        >
          <IconCircle>
            <MdCheckCircleOutline className='text-primary size-6 lg:size-8' />
          </IconCircle>

          <div className='flex flex-col gap-1 w-5/6'>
            <h2 className='text-sm font-medium lg:text-lg'>
              3. Reserva y relajate
            </h2>

            <p className='text-xs lg:text-base'>
              Tu mascota queda en buenas manos, tú sigues con tu día.
            </p>
          </div>
        </motion.div>
      </motion.div>
    </motion.section>
  )
}

export default HowWorks
